...
function getTimeProgress(p){
    ...
    //time since sketch started + random start time (keeps ticking up)
    //colorchange
...
function drawOcean(p){
    var yOffset = 2;
    var yPower = 20;
    var xDist = width/curves; //space between curves; 50
    var xDistOffset = 1;
    var x, y;
    for (var z = 0; z < waveLayers; z++){
        ...
        yOffset*=1.09 + 0.5 ;
        yPower*= 1.15;
        xDistOffset*=1.6; //waves get wider as it's closer in space
    }
    oldTime = p.millis();
}
...
function p1Draw(p) {
    ...
    if (width < (p.windowWidth) || width > (p.windowWidth) 
        || height > (p.windowHeight) || height < (p.windowHeight)){
        width = p.windowWidth;
    ...
    //updating every 40 millis is 24 frames per second
...
function resize(p){
    ...
    if (width < 500){
        //start 3/4 down page
        initialY = p.int(height*(3/4));

    }
    //Desktop
    else{
        //start 2/3 down page
        initialY = p.int(height*(2/3));
    }

}

//Set up top waves sketch
function p1Setup(p) {
    width = p.windowWidth;
    height = p.windowHeight;
    
    randomStartTime  = p.random(0, 100000); //start randomly between from 0 - 100 seconds in timeline
    var cnv = p.createCanvas(width, height); 
    curves = p.int(width/50); //amount of wave curves, less curves, more flat. one curve every 50 px
    create2DNoiseList(p);
    resize(p); //determine how far down page waves start (and how much sky showing above waves)
    drawOcean(p)
